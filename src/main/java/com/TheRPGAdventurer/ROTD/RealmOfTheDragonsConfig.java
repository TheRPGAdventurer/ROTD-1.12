...
public class RealmOfTheDragonsConfig {
	...
	public static double DragonSpeedInGround = 4.7;
	public static double DragonSpeedInAir = 10.7;
	...
	private static void syncconfigs(boolean loadFromConfigFile, boolean readFromConfig) {
		...
		prop = config.get(CATEGORY_MAIN, "Dragon Speed On Air", DragonSpeedInAir);
		prop.setComment("Determines how fasts dragons fly. Dont exaggerate the numbers. 8 or 17 is fine try decimals like 2.5 or something)");
		DragonSpeedInAir = prop.getDouble();
		propOrder.add(prop.getName());
		
		prop = config.get(CATEGORY_MAIN, "Dragon Speed On Ground", DragonSpeedInGround);
		prop.setComment("Determines how fasts dragons walk. Dont exaggerate the numbers. 2 or 3 is fine try decimals like 2.5 or something)");
		DragonSpeedInGround = prop.getDouble();
		propOrder.add(prop.getName());
		
		prop = config.get(CATEGORY_MAIN, "Ticks Per Stage", GET_TICKS_PER_STAGE);
		prop.setComment("Determines how many ticks per lifestage HATCHLING,JUVENILE,ADULT, may increase size");
	...
	public static class ConfigEventHadler {
		...
		public void onEvent(ConfigChangedEvent.OnConfigChangedEvent event) {
			if(event.getModID().equals(RealmOfTheDragons.MODID)) {
				syncFromGui();
			}
		}
	}
