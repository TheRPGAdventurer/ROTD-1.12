...
//    if (entityID == dragon.getControllingPlayer().getEntityId()) return null;
    ...
    final float DAMAGE_PER_HIT_DENSITY = 0.3F;

    float hitDensity = currentHitDensity.getHitDensity();
    if(dragon.getControllingPlayer() != null && entity != dragon.getControllingPlayer()) {entity.setFire((int)(hitDensity * 5));}
    if (currentHitDensity.applyDamageThisTick()) {
    	boolean immunetofire = entity.isImmuneToFire();
    	if(immunetofire) {
      entity.attackEntityFrom(DamageSource.causeMobDamage(dragon), hitDensity * DAMAGE_PER_HIT_DENSITY);} else {
